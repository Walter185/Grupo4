...
export class AddressComponent extends Component{
    ...
    render() {
        ...
        let address = this.props.user && this.props.user.address ? <div className='address'>{this.props.user.address}</div> : <div className='address'>Seleccionar dirección</div>
                    ...
                    {   (this.state).open && 
                        <div className='address-menu-open lite-shadow' style={{display: ((this.state).open ? "flex": "absolute"), top: ((this.state).open ? 0 : '100%')}}>
                            <div className="address-menu-content">
                                ...
                                <h1 className="title">Donde desea recibir su pedido?</h1>
                                ...
                                <button className="btn lite-shadow btn-save" onClick={onSaveLocation}>Guardar ubicación</button>
                            </div>
                        </div>
                    }
                </div>
