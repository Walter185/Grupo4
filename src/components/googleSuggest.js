...
export default class GoogleSuggest extends Component{
    ...
    findDesc = (prediction) => {
        if(!prediction) return "Ningún resultado"
        if(prediction.terms && prediction.terms.length == 1){
            return `${prediction.terms[0].value}`
        }else if(prediction.terms && prediction.terms.length == 2){
                ...
                render={googleMaps =>
                    googleMaps && (
                        <ReactGooglePlacesSuggest
                            ...
                            textNoResults="Ningún local encontrado" // null or "" if you want to disable the no results item
                        ...
                        >
                            <input
                                type="text"
                                value={value}
                                placeholder="Buscar dirección y número"
